...
2. Install the package by calling `colcon build --symlink-install --packages-select table_description`
3. (Re-)Source environment `source install/setup.bash`


> **NOTE:** If you use [RosTeamWorkspace (RTW)](https://rtw.stoglrobotics.de) than instead of the previous three steps, use `cb table_description` command.

Now, launch description test:
```
ros2 launch table_description view_table.launch.xml
```
or
```
ros2 launch table_description view_table.launch.py
```
