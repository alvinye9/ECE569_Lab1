...
2. Install the package by calling `colcon build --symlink-install --packages-select rrbot_description`
3. (Re-)Source environment `source install/setup.bash`


> **NOTE:** If you use [RosTeamWorkspace (RTW)](https://rtw.stoglrobotics.de) than instead of the previous three steps, use `cb rrbot_description` command.

Now, launch description test:
```
ros2 launch rrbot_description view_rrbot.launch.xml
```
or
```
