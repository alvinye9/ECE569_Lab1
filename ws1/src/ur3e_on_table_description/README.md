...
2. Install the package by calling `colcon build --symlink-install --packages-select ur3e_on_table_description`
3. (Re-)Source environment `source install/setup.bash`


> **NOTE:** If you use [RosTeamWorkspace (RTW)](https://rtw.stoglrobotics.de) than instead of the previous three steps, use `cb ur3e_on_table_description` command.

Now, launch description test:
```
ros2 launch ur3e_on_table_description view_ur3e_on_table.launch.xml
```
or
```
